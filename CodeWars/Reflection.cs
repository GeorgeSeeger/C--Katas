...
namespace CodeWars
{
    public static class Reflection
    {
        ...
        public static string ConcatStringMembers(object testObject)
        {
            ...
            var strings = methods.Where(mi => mi.DeclaringType == type && mi.ReturnType.IsInstanceOfType("") && mi.GetParameters().Length == 0)
                .Select(mi => (string) mi.Invoke(testObject, new object[] {})).GroupBy(s => s.Length)
        ...
        {
            Assert.Equal("Test-OutputStark", ConcatStringMembers(new Refl()));
